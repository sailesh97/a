...
router.get('/', passport.authenticate('jwt', { session: false }),
    (req, res) => {
        ...
        Profile.findOne({ user: req.user.id }) // find the user , by the 'user' parameter mentioned in User schema and populate that user object having id of the user with name and avatar too.
...
router.get('/user/:user_id', (req, res) => {
    ...
    Profile.findOne({ user: req.params.user_id })
        ...
        .catch(err => res.status(404).json({ profile: 'There is no profile for this user' })); // If we provide some id that does not exist in our dB , then a error from mongo will appear.
});
